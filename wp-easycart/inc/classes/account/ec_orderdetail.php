...
class ec_orderdetail{
	...
	public $download;// ec_download object
	public $timecheck;
			...
			if ( ( $this->download_timelimit_seconds == 0 || $this->timecheck <= $this->download_timelimit_seconds ) && ( $this->maximum_downloads_allowed  == 0 || $this->download_count <= $this->maximum_downloads_allowed ) ) {
				if ( $this->download->is_amazon_download ) {
					if ( phpversion() >= 5.3 ) {
						...
						$this->mysqli->update_download_count( $this->download->download_id, $this->download_count );
						$amazons3->download_file( $this->download->amazon_key );
					}
					...
					if ( substr( $this->download->download_file_name, 0, 7 ) == "http://" || substr( $this->download->download_file_name, 0, 8 ) == "https://" ) {
						...
						if ( file_exists( $file ) ) {
							...
							esc_attr_e( 'This download is not available on the sever.', 'wp-easycart' );
							die();
						}
					}
				}
